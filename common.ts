...
import { readableStreamToString, PostmarkError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new PostmarkError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new PostmarkError(e, e.response?.data)
                }
                if (innerError instanceof PostmarkError) {
                    // Got "PostmarkError" from the above try block
                    throw innerError;
                }
